...
def _load_governor_classes():
    try:
        from governor_utils import GovernorConfig, ThermalVoltageGovernor

        return GovernorConfig, ThermalVoltageGovernor
    except Exception as exc:  # pragma: no cover - fallback for minimal envs
        print(
            f"[WARN] governor_utils import failed ({exc}); using local governor fallback.",
            file=sys.stderr,
        )
...
def _load_replay_helpers():
    try:
        from governor_utils import load_replay_schedule
    except Exception as exc:
        raise RuntimeError(
            "Failed to import replay schedule loader. Install dependencies for "
            "`unitree_go2_realobs/scripts/rsl_rl/governor_utils.py` (notably numpy, optional pyyaml)."
        ) from exc
