...
"""Train an RSL-RL policy with optional MotorDeg fault-sampling overrides/schedules."""
...
from paper_b_runtime import apply_paper_b_runtime_overrides  # isort: skip
...
parser.add_argument(
    ...
    help="Override MotorDeg fault injection mode for training. 'default' keeps task config as-is.",
)
parser.add_argument(
...
def _set_env_fault_focus_prob(runner: OnPolicyRunner | DistillationRunner, focus_prob: float) -> bool:
    """Update runtime env focus probability so reset-time MotorDeg sampling uses new value."""
    try:
        ...
        if cfg_obj is None:
            return False
        setattr(cfg_obj, "motor_deg_fault_focus_prob", float(focus_prob))
        return True
    except Exception:
        return False
...
def _learn_with_exploration_schedule(
    ...
    Default target profile:
      ...
      - optional focus ramp: update reset-time `motor_deg_fault_focus_prob` on configured iteration boundaries
    """
...
def main(env_cfg: ManagerBasedRLEnvCfg | DirectRLEnvCfg | DirectMARLEnvCfg, agent_cfg: RslRlBaseRunnerCfg):
    ...
    if train_fault_motor_id >= 0:
        setattr(env_cfg, "motor_deg_fault_injection_mode", "single_motor_fixed")
        setattr(env_cfg, "motor_deg_fault_fixed_motor_id", train_fault_motor_id)
        logger.info("[Train] Overriding fixed fault motor: id=%d (single_motor_fixed)", train_fault_motor_id)
    elif train_fault_mode != "default":
        setattr(env_cfg, "motor_deg_fault_injection_mode", train_fault_mode)
        logger.info("[Train] Overriding motor_deg_fault_injection_mode=%s", train_fault_mode)
    if args_cli.train_fault_pair_uniform is not None:
        setattr(env_cfg, "motor_deg_fault_pair_uniform_enable", bool(args_cli.train_fault_pair_uniform))
        logger.info(
            "[Train] Overriding motor_deg_fault_pair_uniform_enable=%s",
    ...
    if args_cli.train_fault_hold_steps is not None:
        ...
        setattr(env_cfg, "motor_deg_fault_hold_steps", int(args_cli.train_fault_hold_steps))
        logger.info("[Train] Overriding motor_deg_fault_hold_steps=%d", int(args_cli.train_fault_hold_steps))
    if args_cli.train_fault_focus_prob is not None:
        ...
        setattr(env_cfg, "motor_deg_fault_focus_prob", focus_prob)
        logger.info("[Train] Overriding motor_deg_fault_focus_prob=%.4f", focus_prob)
    if args_cli.train_fault_pair_weighted_enable is not None:
        setattr(env_cfg, "motor_deg_fault_pair_weighted_enable", bool(args_cli.train_fault_pair_weighted_enable))
        logger.info(
            "[Train] Overriding motor_deg_fault_pair_weighted_enable=%s",
    ...
    if args_cli.train_fault_pair_prob_floor is not None:
        ...
        setattr(env_cfg, "motor_deg_fault_pair_prob_floor", pair_floor)
        logger.info("[Train] Overriding motor_deg_fault_pair_prob_floor=%.4f", pair_floor)
    if args_cli.train_fault_pair_prob_cap is not None:
        ...
        setattr(env_cfg, "motor_deg_fault_pair_prob_cap", pair_cap)
        logger.info("[Train] Overriding motor_deg_fault_pair_prob_cap=%.4f", pair_cap)
    if args_cli.train_fault_pair_target_weights is not None:
        ...
        setattr(env_cfg, "motor_deg_fault_pair_target_weights", pair_target_weights)
        logger.info("[Train] Overriding motor_deg_fault_pair_target_weights=%s", list(pair_target_weights))
    if args_cli.train_fault_pair_adaptive_enable is not None:
        setattr(env_cfg, "motor_deg_fault_pair_adaptive_enable", bool(args_cli.train_fault_pair_adaptive_enable))
        logger.info(
            "[Train] Overriding motor_deg_fault_pair_adaptive_enable=%s",
    ...
    if args_cli.train_fault_pair_adaptive_mix is not None:
        ...
        setattr(env_cfg, "motor_deg_fault_pair_adaptive_mix", adaptive_mix)
        logger.info("[Train] Overriding motor_deg_fault_pair_adaptive_mix=%.4f", adaptive_mix)
    if args_cli.train_fault_pair_adaptive_beta is not None:
        ...
        setattr(env_cfg, "motor_deg_fault_pair_adaptive_beta", adaptive_beta)
        logger.info("[Train] Overriding motor_deg_fault_pair_adaptive_beta=%.4f", adaptive_beta)
    if args_cli.train_fault_pair_adaptive_ema is not None:
        ...
        setattr(env_cfg, "motor_deg_fault_pair_adaptive_ema", adaptive_ema)
        logger.info("[Train] Overriding motor_deg_fault_pair_adaptive_ema=%.4f", adaptive_ema)
    if args_cli.train_fault_pair_adaptive_min_episode_per_pair is not None:
        ...
        setattr(env_cfg, "motor_deg_fault_pair_adaptive_min_episode_per_pair", adaptive_min_ep)
        logger.info(
            "[Train] Overriding motor_deg_fault_pair_adaptive_min_episode_per_pair=%.4f",
            adaptive_min_ep,
        )
    if args_cli.train_fault_pair_adaptive_w_fail is not None:
        ...
        setattr(env_cfg, "motor_deg_fault_pair_adaptive_w_fail", w_fail)
        logger.info("[Train] Overriding motor_deg_fault_pair_adaptive_w_fail=%.4f", w_fail)
    if args_cli.train_fault_pair_adaptive_w_sat is not None:
        ...
        setattr(env_cfg, "motor_deg_fault_pair_adaptive_w_sat", w_sat)
        logger.info("[Train] Overriding motor_deg_fault_pair_adaptive_w_sat=%.4f", w_sat)
    if args_cli.train_fault_pair_adaptive_w_latch is not None:
        ...
        setattr(env_cfg, "motor_deg_fault_pair_adaptive_w_latch", w_latch)
        logger.info("[Train] Overriding motor_deg_fault_pair_adaptive_w_latch=%.4f", w_latch)
    if args_cli.train_fault_pair_adaptive_sat_scale is not None:
        ...
        setattr(env_cfg, "motor_deg_fault_pair_adaptive_sat_scale", sat_scale)
        logger.info("[Train] Overriding motor_deg_fault_pair_adaptive_sat_scale=%.4f", sat_scale)
    ...
    if adaptive_hint_used and args_cli.train_fault_pair_adaptive_enable is None:
        setattr(env_cfg, "motor_deg_fault_pair_adaptive_enable", True)
        logger.info(
            "[Train] Auto-enabling motor_deg_fault_pair_adaptive_enable=True "
            "(adaptive overrides were provided)."
        )
    if args_cli.train_fault_motor_adaptive_enable is not None:
        setattr(env_cfg, "motor_deg_fault_motor_adaptive_enable", bool(args_cli.train_fault_motor_adaptive_enable))
        logger.info(
            "[Train] Overriding motor_deg_fault_motor_adaptive_enable=%s",
    ...
    if args_cli.train_fault_motor_adaptive_topk is not None:
        ...
        setattr(env_cfg, "motor_deg_fault_motor_adaptive_topk", motor_topk)
        logger.info("[Train] Overriding motor_deg_fault_motor_adaptive_topk=%d", motor_topk)
    if args_cli.train_fault_motor_adaptive_min_episode_per_motor is not None:
        ...
        setattr(env_cfg, "motor_deg_fault_motor_adaptive_min_episode_per_motor", motor_min_ep)
        logger.info(
            "[Train] Overriding motor_deg_fault_motor_adaptive_min_episode_per_motor=%.4f",
            motor_min_ep,
        )
    ...
    if motor_adaptive_hint_used and args_cli.train_fault_motor_adaptive_enable is None:
        setattr(env_cfg, "motor_deg_fault_motor_adaptive_enable", True)
        logger.info(
            "[Train] Auto-enabling motor_deg_fault_motor_adaptive_enable=True "
    ...
    if pair_weighted_hint_used and args_cli.train_fault_pair_weighted_enable is None:
        setattr(env_cfg, "motor_deg_fault_pair_weighted_enable", True)
        logger.info(
            "[Train] Auto-enabling motor_deg_fault_pair_weighted_enable=True "
            "(pair/adaptive overrides were provided)."
        )
    focus_ramp_cfg = _resolve_focus_ramp_cfg(args_cli)
    focus_prob_effective = float(getattr(env_cfg, "motor_deg_fault_focus_prob", 0.0))
    ...
    if args_cli.train_fault_focus_pairs is not None:
        ...
        and not bool(getattr(env_cfg, "motor_deg_fault_motor_adaptive_enable", False))
    ):
    ...
    if focus_pairs_override is not None:
        setattr(env_cfg, "motor_deg_fault_focus_pairs", focus_pairs_override)
        logger.info("[Train] Overriding motor_deg_fault_focus_pairs=%s", list(focus_pairs_override))
    if args_cli.train_fault_focus_motor_ids is not None:
        ...
        setattr(env_cfg, "motor_deg_fault_focus_motor_ids", focus_motor_ids)
        logger.info("[Train] Overriding motor_deg_fault_focus_motor_ids=%s", list(focus_motor_ids))
        ...
        setattr(env_cfg, "motor_deg_fault_focus_motor_ids", auto_ids)
        logger.info(
            "[Train] Auto-derived motor_deg_fault_focus_motor_ids from pairs=%s",
            list(auto_ids),
        )
    if focus_ramp_cfg is not None:
        ...
        if args_cli.train_fault_focus_prob is not None:
            logger.info(
                "[Train] Focus ramp is enabled and will override constant motor_deg_fault_focus_prob during training."
            )
        # Set initial cfg value for transparency in dumped env.yaml and first reset.
        setattr(env_cfg, "motor_deg_fault_focus_prob", float(focus_ramp_cfg["start_prob"]))
        logger.info(
