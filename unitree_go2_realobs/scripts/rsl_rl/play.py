...
parser.add_argument(
    ...
    action="store_true",
    default=False,
    help="Color currently faulted leg links in red (env_0 only, based on MotorDeg fault state).",
)
parser.add_argument(
    "--fault_motor_color_vis",
    action="store_true",
    default=False,
    help="Color currently faulted motor link in red (env_0 only, based on MotorDeg fault state).",
)
parser.add_argument(
...
def _make_motor_overload_markers() -> VisualizationMarkers:
    ...
    marker_cfg = VisualizationMarkersCfg(
        prim_path="/Visuals/MotorDeg/motor_overload",
        markers={
...
def _fault_legs_from_motor_deg(base_env) -> torch.Tensor:
    """Read MotorDeg fault state from env_0 and return FR/FL/RR/RL active flags (bool, CPU)."""
    out = torch.zeros((4,), dtype=torch.bool)
    deg_state = base_env.motor_deg_state

    if hasattr(deg_state, "fault_mask"):
        mask = deg_state.fault_mask[0, :12].detach().cpu()
    ...
    if hasattr(deg_state, "fault_motor_id"):
        motor_id = int(deg_state.fault_motor_id[0].item())
        if 0 <= motor_id < 12:
            out[motor_id // 3] = True
    return out


def _fault_motor_from_motor_deg(base_env) -> int:
    """Read MotorDeg fault motor index from env_0. Returns -1 if unavailable."""
    deg_state = base_env.motor_deg_state
    if hasattr(deg_state, "fault_motor_id"):
        motor_id = int(deg_state.fault_motor_id[0].item())
        if 0 <= motor_id < 12:
...
def main(env_cfg: ManagerBasedRLEnvCfg | DirectRLEnvCfg | DirectMARLEnvCfg, agent_cfg: RslRlBaseRunnerCfg):
    ...
    if int(args_cli.force_fault_motor_id) >= 0:
        ...
        if 0 <= motor_id < 12:
            ...
            setattr(env_cfg, "motor_deg_fault_fixed_motor_id", motor_id)
    ...
    if args_cli.video:
        # Tag playback videos with forced MotorDeg settings for easy later comparison.
    ...
    # keep direct handle to base manager env for optional MotorDeg visualization
    ...
    if bool(args_cli.motor_overload_vis):
        if not hasattr(base_env, "robot") or not hasattr(base_env, "motor_deg_state"):
            print("[WARNING] --motor_overload_vis requires MotorDeg env state; skipping marker visualization.")
        else:
    ...
    if bool(args_cli.fault_leg_color_vis):
        if not hasattr(base_env, "robot") or not hasattr(base_env, "motor_deg_state"):
            print("[WARNING] --fault_leg_color_vis requires MotorDeg env state; skipping fault leg coloring.")
        else:
            ...
            if sum(len(v) for v in fault_leg_body_indices.values()) == 0:
                ...
                fault_leg_marker = _make_fault_overlay_marker("/Visuals/MotorDeg/fault_leg_overlay", radius=0.055)
                fault_leg_marker.set_visibility(False)
                fault_leg_vis_enabled = True
                current_fault_legs = _fault_legs_from_motor_deg(base_env)
                last_fault_legs = current_fault_legs.clone()
                leg_names = ("FR", "FL", "RR", "RL")
                active = [leg_names[i] for i in range(4) if bool(current_fault_legs[i].item())]
                if active:
                    print(f"[MotorDeg Vis] Fault leg overlay ON (env_0): {','.join(active)}")
                else:
                    print("[MotorDeg Vis] Fault leg overlay ON (env_0): none")

    if bool(args_cli.fault_motor_color_vis):
        if not hasattr(base_env, "robot") or not hasattr(base_env, "motor_deg_state"):
            print("[WARNING] --fault_motor_color_vis requires MotorDeg env state; skipping fault motor coloring.")
        else:
            # Keep flag name for backward-compat; implementation is overlay marker (no material binding).
            ...
            if sum(1 for _, idx in fault_motor_body_indices.items() if int(idx) >= 0) == 0:
                ...
                fault_motor_marker = _make_fault_overlay_marker("/Visuals/MotorDeg/fault_motor_overlay", radius=0.040)
                fault_motor_marker.set_visibility(False)
                fault_motor_vis_enabled = True
                current_fault_motor = _fault_motor_from_motor_deg(base_env)
                ...
                if 0 <= current_fault_motor < 12:
                    print(f"[MotorDeg Vis] Fault motor overlay ON (env_0): {motor_names[current_fault_motor]} (id={current_fault_motor})")
                else:
                    print("[MotorDeg Vis] Fault motor overlay ON (env_0): none")
    ...
    while simulation_app.is_running():
        ...
        if overload_markers is not None and overload_offsets is not None:
            ...
            case_temp = deg_state.motor_case_temp[0, :12]
            temp_score = torch.clamp((case_temp - 60.0) / 10.0, min=0.0, max=1.0)
            torque_sat = torch.clamp(deg_state.torque_saturation[0, :12], min=0.0, max=1.0)
            ...
            if overload_print_every > 0 and (timestep % overload_print_every == 0):
                print(
                    "[MotorDeg Vis] FR/FL/RR/RL overload = "
        ...
        if fault_leg_vis_enabled and fault_leg_marker is not None and fault_leg_body_indices is not None:
            ...
            if not torch.equal(current_fault_legs, last_fault_legs):
                ...
                active = [leg_names[i] for i in range(4) if bool(current_fault_legs[i].item())]
                if active:
                    print(f"[MotorDeg Vis] Fault leg changed (env_0, overlay): {','.join(active)}")
                else:
                    print("[MotorDeg Vis] Fault leg changed (env_0, overlay): none")
        if (
            ...
            if int(current_fault_motor) != int(last_fault_motor):
                ...
                if 0 <= current_fault_motor < 12:
                    print(f"[MotorDeg Vis] Fault motor changed (env_0, overlay): {motor_names[current_fault_motor]} (id={current_fault_motor})")
                else:
                    print("[MotorDeg Vis] Fault motor changed (env_0, overlay): none")
        if args_cli.video:
