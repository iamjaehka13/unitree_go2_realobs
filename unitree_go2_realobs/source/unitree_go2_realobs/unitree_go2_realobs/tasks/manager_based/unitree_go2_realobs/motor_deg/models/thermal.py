# =============================================================================
# unitree_go2_realobs/motor_deg/models/thermal.py
# Thermal state update helpers for the Go2 MotorDeg model.
# =============================================================================
...
def update_motor_temperature(
    ...
    coil_temp = env.motor_deg_state.coil_temp[env_ids]
    ...
    if (p_coil_watts is not None) or (p_case_watts is not None):
        ...
        if hasattr(env.motor_deg_state, "motor_case_temp"):
            case_temp = env.motor_deg_state.motor_case_temp[env_ids]
        elif hasattr(env.motor_deg_state, "case_temp"):
            case_temp = env.motor_deg_state.case_temp[env_ids]
        else:
        ...
        if hasattr(env.motor_deg_state, "motor_case_temp"):
            env.motor_deg_state.motor_case_temp[env_ids] = new_case
        if hasattr(env.motor_deg_state, "case_temp"):
            env.motor_deg_state.case_temp[env_ids] = new_case
        if hasattr(env.motor_deg_state, "case_temp_derivative"):
            safe_dt = dt if dt > 1e-6 else 1e-6
            env.motor_deg_state.case_temp_derivative[env_ids] = (new_case - case_temp) / safe_dt
    ...
    if hasattr(env.motor_deg_state, "motor_case_temp"):
        env.motor_deg_state.motor_case_temp[env_ids] = torch.clamp(new_temp - 5.0, min=T_AMB)
    if hasattr(env.motor_deg_state, "case_temp"):
        env.motor_deg_state.case_temp[env_ids] = torch.clamp(new_temp - 5.0, min=T_AMB)
    if hasattr(env.motor_deg_state, "case_temp_derivative"):
