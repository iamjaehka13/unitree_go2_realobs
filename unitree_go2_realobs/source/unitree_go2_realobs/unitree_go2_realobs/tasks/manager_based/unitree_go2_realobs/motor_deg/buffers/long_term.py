# unitree_go2_realobs/motor_deg/buffers/long_term.py
...
class LongTermHealthBuffer:
    """
    [MotorDeg Core] 로봇의 장기적 건강 상태 변화와 열화 추이를 추적하는 통계 버퍼.
    ...
    def update(self, env: ManagerBasedRLEnv, dt: float, env_ids=None):
        ...
        if torch.any(trigger_mask):
            ...
            self._take_snapshot(env.motor_deg_state.fatigue_index, triggered_env_ids)
