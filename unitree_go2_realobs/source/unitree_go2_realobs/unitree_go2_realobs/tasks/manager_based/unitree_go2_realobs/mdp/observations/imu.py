# unitree_go2_realobs/mdp/observations/imu.py
#
# IMU observation helpers with optional degradation-aware drift modeling.
...
from ...motor_deg.utils import compute_kinematic_accel
try:
    from ...motor_deg.constants import GRAVITY_VAL
except ImportError:
...
def base_ang_vel_motor_deg(
    ...
    if hasattr(env, "motor_deg_state"):
        avg_temp = torch.mean(env.motor_deg_state.coil_temp[env_ids], dim=1, keepdim=True)
...
def projected_gravity_motor_deg(
    env: ManagerBasedEnv,
    env_ids: torch.Tensor | slice = slice(None),
...
# =============================================================================
# 2. MotorDeg Optimized Observations (Kinematic Acceleration)
# =============================================================================

def base_lin_accel_motor_deg(
    env: ManagerBasedEnv,
    env_ids: torch.Tensor | slice = slice(None),
    sensor_cfg: SceneEntityCfg = SceneEntityCfg("base_imu"),
    asset_cfg: SceneEntityCfg = SceneEntityCfg("robot")
    ...
    """
    [MotorDeg Standardized] 중력을 제거한 '순수 동적 가속도(Dynamic Acceleration)' 관측.
    Returns:
...
def base_lin_accel_motor_deg_with_drift(
    env: ManagerBasedEnv,
    env_ids: torch.Tensor | slice = slice(None),
    sensor_cfg: SceneEntityCfg = SceneEntityCfg("base_imu"),
    asset_cfg: SceneEntityCfg = SceneEntityCfg("robot")
    ...
    accel_g = base_lin_accel_motor_deg(env, env_ids, sensor_cfg, asset_cfg)
...
def orientation_error_motor_deg(
    env: ManagerBasedEnv,
    env_ids: torch.Tensor | slice = slice(None),
    asset_cfg: SceneEntityCfg = SceneEntityCfg("robot")
) -> torch.Tensor:
...
# =============================================================================
# 3. Debugging & MotorDeg Analysis Tool
# =============================================================================
...
def debug_imu_precision(
    ...
    drifted_accel = base_lin_accel_motor_deg_with_drift(env, env_ids, sensor_cfg, asset_cfg)
    ...
    # 3. 온도 상태 확인
    if hasattr(env, "motor_deg_state"):
        avg_temp = torch.mean(env.motor_deg_state.coil_temp[env_ids]).item()
    ...
    print(f"\n[MotorDeg IMU INTEGRITY] Step {step} | {temp_status}")
