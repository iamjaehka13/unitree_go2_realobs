# unitree_go2_realobs/mdp/observations/proprioception.py
#
# MotorDeg-enhanced proprioceptive observation helpers.
# Joint velocity channels reuse the same measurement noise cached in `MotorDegState`
...
# 이 모듈의 MotorDeg-전용 함수만 export합니다.
__all__ = [
    "joint_pos_rel_motor_deg",
    "joint_vel_motor_deg",
    "joint_acc",
    "joint_pos_limit_normalized",
    "joint_torques_applied",
    "debug_proprioception_motor_deg",
]
...
def joint_pos_rel_motor_deg(env: ManagerBasedRLEnv, env_ids: torch.Tensor | slice = slice(None), asset_cfg: SceneEntityCfg = SceneEntityCfg("robot")) -> torch.Tensor:
    """
    [MotorDeg Enhanced] 중앙 MotorDeg 상태(motor_deg_state)와 동기화된 관절 위치 관측.
    ...
    # 2. MotorDeg State 기반 오차 주입 (SSOT)
    if hasattr(env, "motor_deg_state") and hasattr(env.motor_deg_state, "encoder_meas_pos"):
        measured_pos_abs = env.motor_deg_state.encoder_meas_pos[env_ids][:, joint_ids]
    elif hasattr(env, "motor_deg_state"):
        cached_offset = env.motor_deg_state.encoder_offset[env_ids]
        cached_noise = env.motor_deg_state.encoder_noise[env_ids]
...
def joint_vel_motor_deg(env: ManagerBasedRLEnv, env_ids: torch.Tensor | slice = slice(None), asset_cfg: SceneEntityCfg = SceneEntityCfg("robot")) -> torch.Tensor:
    """
    [MotorDeg Enhanced] 관절 각속도 (Normalized).
    ...
    if hasattr(env, "motor_deg_state") and hasattr(env.motor_deg_state, "encoder_meas_vel"):
        measured_vel = env.motor_deg_state.encoder_meas_vel[env_ids][:, joint_ids]
    elif hasattr(env, "motor_deg_state"):
        # Actuator가 보는 것과 동일한 속도 노이즈 주입
        vel_noise = getattr(env.motor_deg_state, "encoder_vel_noise", 0.0)
...
def joint_acc(env: ManagerBasedRLEnv, env_ids: torch.Tensor | slice = slice(None), asset_cfg: SceneEntityCfg = SceneEntityCfg("robot")) -> torch.Tensor:
    """[MotorDeg Critical] 관절 각가속도 (Normalized by ACC_SCALE)."""
    asset = env.scene[asset_cfg.name]
    joint_ids = asset_cfg.joint_ids
    if joint_ids is None:
        joint_ids = slice(None)
...
def joint_torques_applied(env: ManagerBasedRLEnv, env_ids: torch.Tensor | slice = slice(None), asset_cfg: SceneEntityCfg = SceneEntityCfg("robot")) -> torch.Tensor:
    """[MotorDeg Critical] 실제 인가된 토크 / Effort Limits."""
    asset = env.scene[asset_cfg.name]
    joint_ids = asset_cfg.joint_ids
    if joint_ids is None:
        joint_ids = slice(None)
...
# =============================================================================
# [Debug] Real-time MotorDeg Monitoring
# =============================================================================
def debug_proprioception_motor_deg(env: ManagerBasedRLEnv, env_ids: torch.Tensor | slice = slice(None), asset_cfg: SceneEntityCfg = SceneEntityCfg("robot")) -> torch.Tensor:
    """
    [MotorDeg Debug Dashboard]
    SSOT 검증: motor_deg_state의 노이즈가 실제로 관측값에 반영되었는지 육안 확인 가능.
    """
    ...
    if hasattr(env, "motor_deg_state"):
        offset_val = env.motor_deg_state.encoder_offset[0]
        pos_noise = env.motor_deg_state.encoder_noise[0]
        vel_noise = getattr(env.motor_deg_state, "encoder_vel_noise", torch.zeros_like(true_vel))[0]
    ...
    print(f"\n[Isaac Lab MotorDeg Proprioception] Step: {step}")
