# =============================================================================
# unitree_go2_realobs/mdp/observations/contact.py
# Contact-derived MotorDeg observation helpers for CoP, impact, and vibration metrics.
# =============================================================================
# Purpose:
#   Compute MotorDeg (Physical Health Monitoring) metrics using foot contact data.
...
def _get_total_forces_in_base_frame(
    ...
    """
    # [MotorDeg Truth] 기계적 부하는 필터링되지 않은 총 외력(net_forces)을 따른다.
...
def _calculate_cop_body_frame_vectorized(
    ...
    if data.contact_pos_w is None or data.contact_pos_w.numel() == 0:
        if not _LOG_FLAGS["contact_points"]:
            logging.error(
                "[MotorDeg Critical] 'contact_pos_w' empty. "
    ...
    if not hasattr(data, "force_matrix_w") or data.force_matrix_w is None:
        if not _LOG_FLAGS["force_matrix"]:
            logging.error(
                "[MotorDeg Strict] 'force_matrix_w' missing. "
...
# =============================================================================
# Impact (Landing Shock) - MotorDeg Metric
# =============================================================================
def contact_impact(
    env: ManagerBasedEnv,
    sensor_cfg: SceneEntityCfg,
    ...
    if (
        ...
        if not _LOG_FLAGS["short_history"]:
            logging.warning(
                "[MotorDeg Critical] Contact history < 2. "
...
def weighted_contact_acceleration(
    ...
    """
    MotorDeg Metric: |IMU dynamic acceleration| × contact load ratio
    ...
    try:
        ...
        if not _LOG_FLAGS["missing_imu"]:
            logging.error(
                f"[MotorDeg Error] IMU sensor '{imu_cfg.name}' not found in Scene."
            )
