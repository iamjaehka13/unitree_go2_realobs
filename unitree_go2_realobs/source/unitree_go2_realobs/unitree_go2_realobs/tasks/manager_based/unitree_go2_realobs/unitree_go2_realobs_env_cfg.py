...
class RealObsObservationsCfg:
    ...
    class PolicyCfg(ObsGroup):
        ...
        # Reference locomotion policy core + measurable degradation channels
        ...
        # deg_mdp.*_motor_deg already injects degradation-side sensor corruption.
        ...
        energy_budget = ObsTerm(
            func=deg_mdp.available_voltage_budget_realobs,
            params={"cutoff_voltage": 24.5},
            noise=None,
        )
        thermal_stress = ObsTerm(
            func=deg_mdp.thermal_stress_realobs,
            params={
    ...
    class CriticCfg(ObsGroup):
        # Reference critic core + measurable degradation channels. No corruption.
...
class RealObsRewardsCfg:
    """Safety rewards built from measurable-only degradation proxies."""
    ...
    thermal_safety = RewTerm(
        func=deg_mdp.thermal_margin_reward_realobs,
        weight=0.03,
        params={
            "std": 1.25,
            "limit_temp": 70.0,
...
class RealObsTerminationsCfg(TerminationsCfg):
    ...
    # Reason:
    # - Current protocol uses thermal safety as soft constraint (reward + MotorDeg dynamics),
